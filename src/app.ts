...
import { ClassParser, LibraryParser, MethodParser, NamespaceParser } from "./parser";
...
export function parseDefinitions(config: IConfigDef) {
    if (!config) {
        config = {};
    }
    ...
    function pathJoin(parts: string[], sep?: string) {
        ...
        let replace: RegExp = new RegExp(separator + "{1,}", "g");
